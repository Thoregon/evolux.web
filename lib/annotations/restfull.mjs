...
export default class Restfull {
    ...
    isAuthorized(req, data) {
        ...
        if (this.home.isAuthorized(req.apiKey, req.url, data)) return true;
        return false;
    }
    ...
    serve(wwwroot, target, config) {
        ...
        const root = this.root ? '' : universe.WWWAPI ?? '/api';
        ...
        Object.entries(endpoints).forEach(([mthname, endpointconfig]) => {
            ...
            // const resheaders = Object.fromEntries([endpointconfig.produces.split(':').map(part => part.replaceAll("'", "").trim())]);
            ...
            const needsAuthentication = !!endpointconfig.auth;
            try {
                wwwroot[mth](path, async (req, res, data, utils) => {
                    try {
                        if (needsAuthentication && !this.isAuthorized(req, data)) {
                            console.error(">> RESTRouter::handle NOT AUTHORIZED");
                            return res.status(401).send({ error: 401, message: 'Not authorized' });
                        }
                        const url = req.url;
                        data.remoteAddress = req.header('x-forwarded-for') ?? req.socket?.remoteAddress;
                        try { console.log("-- Restfull", mth, url, path, data ? JSON.stringify({ params: data.params, query: data.query }) : "") } catch (err) { console.log(">> Restfull", mth, url, path) }
                        data.headers       = req.headers;
                        data.req           = req;
                        const result       = await target[mthname](data);

                        // Object.entries(produces).forEach(([header, value]) => res.set(header, value));
                        if (produces) res.type(whichProduce(produces));
                        res.send(result);
                    } catch (e) {
                        console.log(">> RESTInterface", e.stack ? e.stack : e);
...
function whichProduce(contentType) {
    // todo: add more content types
    if (contentType.indexOf('application/json') > -1) return '.json'
    if (contentType.indexOf('text/html') > -1) return '.html'
    return contentType;
}
...
export const Path = (target, mthname, meta, params) => {
    const restfull  = getRestfull(meta);
    // restfull.prefix = restfull.isApi ? `api/${params}` : params;
