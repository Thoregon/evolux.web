...
export default class Router {
     ...
     * @param {Function} authfn     ... if provided will be invoked for authorization
     */
    post(route, fn, { authfn, raw = false } = {}) {
        return this.handle(route, 'post', fn, { authfn, raw });
    }
     ...
     *  handle method
     */
    handle(route, mth, fn, { authfn, raw = false } = {}) {
        ...
        const handlerfn = async (req, res) => {
            ...
            try {
                ...
                const serverProperties                 = this._getServerProperties(req);
                if ((authfn && !authfn({ auth, url, params, query, content, serverProperties }))
                    || (this.isAuthorized && !this.isAuthorized({ auth, url, params, query, content, serverProperties }))) {
                    utils.sendError(401, 'Not authorized');
                } else {
                    return await fn(req, res, { auth, params, query, content, req, res, serverProperties }, utils);
                }
            } catch (e) {
        ...
        if (raw) {
            this.service[mth](url, this.express.raw({type: '*/*'}), handlerfn);
            // this.service[mth](url, this.express.raw({type: 'application/json'}), handlerfn);
            console.log("-- WebRouter raw", mth, url);
        } else {
            this.service[mth](url, handlerfn);
        }
    }

    _getServerProperties(req) {
        const protocol = req.protocol;               // 'http' or 'https'
        const path = req.originalUrl;         // path + query
        ...
        if (hostHeader && hostHeader.includes(':')) {
            [host, port] = hostHeader.split(':');
        }

        const properties = { protocol, host, port, path };
        return properties;
    }
    ...
    async bodyAsObject(req) {
        switch (req.headers['content-type']) {
            case 'application/x-www-form-urlencoded':
                let content = req.body; // await parse.form(req);
                return content;
            default:
                let body = req.body; // await parse.text(req, {limit: '100kb'});
                if (!body) return;
                // body = body.trim();
